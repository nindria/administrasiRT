<script setup lang="ts">
import BaseInput from '@/components/form/BaseInput.vue';
import Label from '@/components/ui/label/Label.vue';
import AppLayout from '@/layouts/AppLayout.vue';
import { BreadcrumbItem, Role } from '@/types';
import { Head, Link, useForm } from '@inertiajs/vue3';
import { ChevronLeft } from 'lucide-vue-next';

const breadcrumbs: BreadcrumbItem[] = [
    {
        title: 'Roles Edit',
        href: '/roles',
    },
];

const props = defineProps<{
    role: Role;
    permissions: { id: number; name: string }[];
    rolePermissions: string[];
}>();

const form = useForm({
    name: props.role.name,
    permissions: props.rolePermissions || [], // akan menampung id permission yang dipilih
});
</script>

<template>
    <Head title="Role" />

    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
            <h1 class="text-2xl font-bold">Edit Data Peran</h1>

            <form @submit.prevent="form.put(`/roles/${props.role.id}`)" class="grid gap-4">
                <BaseInput label="Name" name="name" v-model:value="form.name" placeholder="Name" :message="form.errors.name" />
                <Label class="mb-2">Permissions:</Label>
                <div class="grid grid-cols-2 gap-2">
                    <Label v-for="permission in permissions" :key="permission.id" class="flex items-center space-x-2">
                        <input
                            type="checkbox"
                            class="h-4 w-4 rounded border-gray-300 bg-gray-100 text-blue-600 focus:ring-2 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:ring-offset-gray-800 dark:focus:ring-blue-600"
                            :value="permission.name"
                            v-model="form.permissions"
                        />
                        <span class="text-gray-800 capitalize">{{ permission.name }}</span>
                        <!-- error message -->
                    </Label>
                </div>
                <div class="mt-4 flex justify-end gap-2">
                    <Link
                        href="/roles"
                        class="inline-flex w-24 items-center rounded-lg bg-gradient-to-r from-blue-400 via-blue-500 to-blue-600 px-4 py-2 text-white hover:bg-gradient-to-br focus:ring-4 focus:ring-blue-300 focus:outline-none dark:focus:ring-blue-800"
                    >
                        <ChevronLeft class="mr-1 h-4 w-4" />
                        Back
                    </Link>
                    <button
                        type="submit"
                        class="w-full rounded-lg bg-blue-700 px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 focus:outline-none sm:w-auto dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
                    >
                        Submit
                    </button>
                </div>
            </form>
        </div>
    </AppLayout>
</template>
