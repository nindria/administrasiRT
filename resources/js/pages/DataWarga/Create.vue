    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
            <div class="pb-4">
                <Link
                    href="/datawarga"
                    class="me-2 mb-2 inline-flex items-center rounded-lg bg-gradient-to-r from-blue-400 via-blue-500 to-blue-600 px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-gradient-to-br focus:ring-4 focus:ring-blue-300 focus:outline-none dark:focus:ring-blue-800"
                >
            ...
            <form @submit.prevent="submit" enctype="multipart/form-data">
                <div class="grid gap-4">
                    ...
                    <div>
                        ...
                        <Input
                            v-model="form.husband_birth_place"
                            class="uppercase"
                            @input="
                                (e: Event) => {
                                    const target = e.target as HTMLInputElement;
                    ...
                    <div>
                        ...
                        <select v-model="form.married_status" class="w-full rounded border">
                    ...
                    <div v-if="form.married_status === 'menikah'">
                        ...
                        <div v-for="(child, index) in form.children_data" :key="index" class="mt-2 grid gap-2 border-l border-gray-300 pl-4">
                    ...
                    <div v-for="(member, index) in form.other_family_members" :key="index" class="grid gap-2">
                        <div class="flex items-center gap-2">
                            ...
                            <div class="flex-1">
                                ...
                                <Input v-model="member.status" :id="`member_status_${index}`" placeholder="Status dalam keluarga" />
                            </div>
                            <Button
                                type="button"
                                variant="destructive"
                                @click="removeOtherFamilyMember(index)"
                                v-if="form.other_family_members.length > 1"
                                class="mb-1 self-end"
                                >Hapus</Button
                            >
                        </div>
                    </div>
                    <Button type="button" variant="secondary" class="mt-2" @click="addOtherFamilyMember"> Tambah Anggota </Button>

                    <!-- Status Tempat Tinggal -->
                    <div>
                        <Label for="residence_status">Status Tempat Tinggal</Label>
                        <select v-model="form.residence_status" class="w-full rounded border">
